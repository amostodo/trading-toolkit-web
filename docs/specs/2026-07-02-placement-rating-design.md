...
| 安全垫得分 | 30% | `score = min(safetyPad / 10, 1)` | 前端按当前预期上市溢价率派生 |
...
安全垫由前端按当前预期上市溢价率计算。该值默认 30%，仅允许 30% 至 100% 的 10% 档位，并只用于情景比较，不代表市场行情、已验证收益或可执行结果。
...
- **安全垫**: `安全垫 = 预估收益(1000×当前预期上市溢价率) / (实际获配每手成本) × 100%`
...
- 安全垫按当前全局预期上市溢价率派生；接口安全垫仅保留为源数据
